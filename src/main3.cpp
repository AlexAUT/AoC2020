...
std::array<std::string_view, 1000> data = {
...
int main() {
  ...
  for (auto &row : data) {
    ...
    auto old_pos = row.find_first_of('-');
    int pos1;
    std::from_chars(row.data(), row.data() + old_pos, pos1);

    auto new_pos = row.find_first_of(' ');
    int pos2;
    std::from_chars(row.data() + old_pos + 1, row.data() + new_pos, pos2);

    std::cout << "MinMax: " << pos1 << " " << pos2 << std::endl;

    auto space_pos = row.find_first_of(' ');
    char sign = row.data()[space_pos + 1];

    auto second_space_pos = row.find_first_of(' ', space_pos + 1);

    auto *word_start = row.data() + second_space_pos;

    if ((word_start[pos1] == sign) ^ (word_start[pos2] == sign)) {
      wrong_counter++;
    }
  }

  std::cout << "Found " << wrong_counter << " wrong entries" << std::endl;
}
